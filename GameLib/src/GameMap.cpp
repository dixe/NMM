...
namespace GameLib
{
    ...
    void MapDescription(char* buffer);
    ...
    std::string GameMap::MapDescription()
    {
        std::string retString;
        for( int i = 0; i < (int) mapNodes.size(); i++)
        {
            Node node = mapNodes[i];
            if(node.populated != FREE)
            {
                char desbuf[5];
                char letter = (char) (node.position.letter + 'A');
                char num = node.position.number + '1';
                char pNum = node.populated + '0';
                sprintf_s(desbuf,"%c%c%c ", pNum, letter, num);
                
                retString.append(desbuf);
            }
        }
